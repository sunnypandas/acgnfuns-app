...
export default class ComicMangaScreen extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  static navigationOptions = {
    header: <HeaderBar title={'漫画详情'} />,
  };
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'acgn/comicMangaByUrlFetch',
      payload: {
        url: this.props.navigation.getParam('url', ''),
      },
    }).then(() => {
      const { acgn } = this.props;
      const tempMedia = acgn.comicManga;
      const mediaObj = {
        url: tempMedia.url,
